...
| private\_route | Private Destination CIDR blocks for NACL definition | <pre>list(object({<br>    cidr = string<br>    protocol = optional(string, "tcp")<br>    nacl_inbound_ports = list(number) <br>    nacl_outbound_ports = list(number)<br>  }))</pre> | n/a | yes |
